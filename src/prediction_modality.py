...
#model_audio.show()
scores = cross_val_score(rf, dev[x], dev.binary, cv=10, scoring='neg_mean_squared_error')
math.sqrt(abs(scores.mean()))

scores = cross_val_score(rf, dev[x], dev.binary, cv=10, scoring='neg_mean_absolute_error')
abs(scores.mean())
